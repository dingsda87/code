...
class BaseMapper extends APFObject
{
    ...
    protected function generateMappingItem($objectName, $objectSection)
    {
        ...
        array_walk($objectSection, function (&$value, $key) {
            switch (strtolower(substr($value, 0, 3))) {
                case 'int':
                    $value = DatabaseConnection::PARAM_INTEGER;
                    break;
                case 'tex':
                case 'var':
                    $value = DatabaseConnection::PARAM_STRING;
                    break;
                case 'flo':
                    $value=DatabaseConnection::PARAM_FLOAT;
                    break;
                default:
                    $value=DatabaseConnection::PARAM_STRING;
